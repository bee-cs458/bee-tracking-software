...
export default function UsersTable(props) {
    ...
    async function handleConfirmButton() {
        ...
        await promoteOrDemoteAdvancedUser(userPendingChange.user_id).then(
            (result) => {
            ...
            {(users != null && users.length > 0) ? (
                <div>
                    <Table striped bordered>
                        ...
                        <tbody>
                            {users.map((user) => (
                                <UserRow key={user.user_id} lastUserPromoted={lastUserPromoted} setLastUserPromoted={setLastUserPromoted} popModal={popModal} item={user}></UserRow>
                            ))}
                        </tbody>
                    </Table>
                    <Modal
                    ...
                    >
                        ...
                        <Modal.Body>
                                {userPendingChange.user_id  !== undefined ? <><p>
                                    You are about to {
                        ...
                        <Modal.Footer>
                            <Button variant="primary" onClick={handleConfirmButton} disabled={loading}>Confirm</Button>

                            <Button variant="secondary" onClick={handleCloseButton}>Cancel</Button>
